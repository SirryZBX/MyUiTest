...
def _get_html_response(url: str, session: PipSession) -> Response:
    """Access an HTML pages with GET, and return the response.
    ...
    logger.debug('Getting pages %s', redact_auth_from_url(url))
    ...
    resp = session.get(
        ...
        headers={
            ...
            # minimize traffic sent in cases where the pages hasn't
...
def _create_link_from_element(
    ...
    """
    Convert an anchor element in a simple repository pages to a Link.
    """
...
def with_cached_html_pages(
    ...
    `pages` has `pages.cache_link_parsing == False`.
    """
...
class HTMLPage:
    """Represents one pages, along with its URL"""
    ...
    def __init__(
        ...
        :param cache_link_parsing: whether links parsed from this pages's url
...
def _get_html_page(
    ...
    try:
        ...
        logger.warning(
            'Skipping pages %s because it looks like an archive, and cannot '
            'be checked by a HTTP HEAD request.', link,
        )
    except _NotHTML as exc:
        logger.warning(
            'Skipping pages %s because the %s request got Content-Type: %s.'
...
class LinkCollector:
    ...
    def fetch_page(self, location: Link) -> Optional[HTMLPage]:
        """
        Fetch an HTML pages containing package links.
        """
